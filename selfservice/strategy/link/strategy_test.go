...
func TestStrategy(t *testing.T) {
	...
	t.Run("description=should recover an account", func(t *testing.T) {
		...
		message := testhelpers.CourierExpectMessage(t, reg, "recover@ory.sh", "Recover your account")
		assert.Contains(t, message.Body, "please recover your account by clicking the following link")
	...
	t.Run("description=should not be able to use an outdated link", func(t *testing.T) {
		viper.Set(configuration.ViperKeySelfServiceLifespanRecoveryRequest, time.Millisecond*10)
		t.Cleanup(func() {
